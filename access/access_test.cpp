...
bool MyApp::OnInit()
{
    ...
    auto *frame = new MyFrame("AccessTest wxWidgets App", wxPoint(50, 50), wxSize(450, 340));
...
{
public:
    explicit SplitterWindowAccessible(wxWindow *win) : wxWindowAccessible(win)
    {}
    ...
    wxAccStatus Navigate(wxNavDir navDir, int fromId, int *toId, wxAccessible **toObject) wxOVERRIDE;

    // Gets the number of children.
    wxAccStatus GetChildCount(int *childCount) wxOVERRIDE;

    // Gets the specified child (starting from 1).
    // If *child is nullptr and return value is wxACC_OK,
    // this means that the child is a simple element and
    // not an accessible object.
    wxAccStatus GetChild(int childId, wxAccessible **child) wxOVERRIDE;

    // Gets the parent, or nullptr.
    wxAccStatus GetParent(wxAccessible **parent) wxOVERRIDE;
    ...
    wxAccStatus GetDefaultAction(int childId, wxString *actionName) wxOVERRIDE;

    // Returns the description for this object or a child.
    wxAccStatus GetDescription(int childId, wxString *description) wxOVERRIDE;

    // Returns help text for this object or a child, similar to tooltip text.
    wxAccStatus GetHelpText(int childId, wxString *helpText) wxOVERRIDE;

    // Returns the keyboard shortcut for this object or child.
    // Return e.g. ALT+K
    wxAccStatus GetKeyboardShortcut(int childId, wxString *shortcut) wxOVERRIDE;
    ...
    // or child.
    wxAccStatus GetValue(int childId, wxString *strValue) wxOVERRIDE;

    // Selects the object or child.
    wxAccStatus Select(int childId, wxAccSelectionFlags selectFlags) wxOVERRIDE;

    // Gets the window with the keyboard focus.
    // If childId is 0 and child is nullptr, no object in
    // this sub-hierarchy has the focus.
    ...
    : wxFrame(nullptr, wxID_ANY, title, pos, size, style)
{
  ...
  SetStatusText("Welcome to wxWidgets!");
    ...
    auto *listBox = new wxListBox(splitter, wxID_ANY);
...
void MyFrame::OnQuery(wxCommandEvent &WXUNUSED(event))
{
    ...
    if (accessibleFrame) {
        ...
        long obtained = 0;
        auto *var = new VARIANT[childCount];
        int i;
...
void MyFrame::LogObject(int indent, IAccessible *obj) // NOLINT(*-no-recursion)
{
    ...
    for (i = 1; i <= childCount; i++) {
        ...
        if (S_OK == obj->get_accChild(var, &pDispatch) && pDispatch) {
