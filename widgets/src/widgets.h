...
#ifndef WX_WIDGETS_SAMPLE_H_
#define WX_WIDGETS_SAMPLE_H_
...
enum
{
    ...
    // renderer'
    NATIVE_PAGE = 0,
    UNIVERSAL_PAGE [[maybe_unused]] = NATIVE_PAGE,
    GENERIC_PAGE,
    PICKER_PAGE,
    COMBO_PAGE,
    WITH_ITEMS_PAGE,
...
{
public:
    WidgetsPage(WidgetsBookCtrl *book, wxImageList *imageList, const char *const icon[]);

    // return the control shown by this page
    [[nodiscard]] virtual wxWindow *GetWidget() const = 0;

    // return the control shown by this page, if it supports text entry interface
    [[nodiscard]] virtual wxTextEntryBase *GetTextEntry() const { return nullptr; }
    ...
    [[nodiscard]] virtual Widgets GetWidgets() const
    {
        Widgets widgets;
...
protected:
    ...
    // create a horizontal sizer containing the given control and the text ctrl
    // (pointer to which will be saved in the provided variable if not NULL)
    // with the specified id
    wxSizer *CreateSizerWithText(wxControl *control, wxWindowID id = wxID_ANY, wxTextCtrl **ppText = nullptr);

    // create a sizer containing a label and a text ctrl
    wxSizer *CreateSizerWithTextAndLabel(const wxString &label, wxWindowID id = wxID_ANY, wxTextCtrl **ppText = nullptr);

    // create a sizer containing a button and a text ctrl
    wxSizer *CreateSizerWithTextAndButton(wxWindowID idBtn, const wxString &labelBtn, wxWindowID id = wxID_ANY, wxTextCtrl **ppText = nullptr);
...
{
public:
    typedef WidgetsPage *(*Constructor)(WidgetsBookCtrl *book, wxImageList *imageList);

    // our ctor
    WidgetsPageInfo(Constructor ctor, const wxString &label, int categories);

    // accessors
    [[nodiscard]] const wxString &GetLabel() const { return m_label; }
    [[nodiscard]] int GetCategories() const { return m_categories; }
    [[nodiscard]] Constructor GetCtor() const { return m_ctor; }
    [[nodiscard]] WidgetsPageInfo *GetNext() const { return m_next; }
...
#define IMPLEMENT_WIDGETS_PAGE(classname, label, categories)                        \
    WidgetsPage *wxCtorFor##classname(WidgetsBookCtrl *book, wxImageList *imaglist) \
    {                                                                               \
        return new classname(book, imaglist);                                       \
    }                                                                               \
    WidgetsPageInfo classname::ms_info##classname(wxCtorFor##classname, label, ALL_CTRLS | categories)

#endif// WX_WIDGETS_MYSAMPLE_H_
