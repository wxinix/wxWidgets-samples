...
class wxConstraintOp: public wxObject
{
 public:
   ...
   explicit wxConstraintOp(int typ)
   {
     opType = typ;
     edge = wxLeft;
     win = nullptr;
   ...
   wxConstraintOp(const wxConstraintOp& op) //NOLINT
   {
     opType = op.opType;
     edge = op.edge;
  ...
  wxConstraintOp operator = (const int a_value);

  friend wxConstraintOp operator % (int perCent, const wxConstraintOp& arg2);
...
wxConstraintOp wxConstraintOp::operator = (const int a_value)
{
  wxConstraintOp op3(wxCONSTRAINT_OP_REL);
